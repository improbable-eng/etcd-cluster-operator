...
func (r *EtcdPeerReconciler) getClientCertificates(ctx context.Context, clusterName, namespace string) ([]byte, []byte, []byte, error) {
	secret := &corev1.Secret{}
...
func (r *EtcdPeerReconciler) Reconcile(req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	if peer.Spec.TLS != nil && peer.Spec.TLS.Enabled {
		clientCaCert, clientCert, clientKey, err := r.getClientCertificates(ctx, peer.Spec.ClusterName, peer.Namespace)
		if err != nil {
