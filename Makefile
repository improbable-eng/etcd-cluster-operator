...
manifests-image:  # Build the manifests docker image.
	docker build -t $(DOCKER_IMAGE_MANIFESTS) --build-arg OPERATOR_IMAGE=$(DOCKER_IMAGE_CONTROLLER) --build-arg PROXY_IMAGE=$(DOCKER_IMAGE_PROXY) -f manifests.Dockerfile .
